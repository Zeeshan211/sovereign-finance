/* Sovereign Finance Add Transaction Form v0.4.4
Phase 6D Command Centre enforcement reason cleanup.
Contract:
...
? 'Command Centre has blocked transaction.save on this page. Viewing and editing fields is still allowed for diagnosis.'
...
if (selectedType === 'transfer') ok = ok && !!to && to !== from;
btn.disabled = !ok;
if (!saveGate.allowed) setSubmitText('Blocked by Command Centre');
else if (!submitting) setSubmitText('Save Transaction');
}
...
console.warn('[add v0.4.4] /api/accounts failed:', e1.message);
try {
...
console.warn('[add v0.4.4] store account fallback failed:', e2.message);
}
}
...
console.warn('[add v0.4.4] /api/categories failed:', e.message);
categories = [];
categoriesLoaded = false;
}
}
...
if (routeFrom && [...source.options].some(o => o.value === routeFrom.id)) source.value = routeFrom.id;
else if (old && [...source.options].some(o => o.value === old)) source.value = old;
}
...
if (!saveGate.allowed) throw new Error('Command Centre blocked transaction.save: ' + (saveGate.reason || 'No reason returned.'));
const amount = parseFloat(($('amountInput') || {}).value || '0');
const from = ($('accountSelect') || {}).value || '';
const to = ($('transferToSelect') || {}).value || '';
...
if (payload.transferToAccountId) body.transfer_to_account_id = payload.transferToAccountId;
...
try {
let result;
if (window.store && typeof window.store.addTransaction === 'function') result = await window.store.addTransaction(payload);
else result = await directAdd(payload);
...
setTimeout(() => { window.location.href = '/transactions.html'; }, 650);
} catch (err) {
