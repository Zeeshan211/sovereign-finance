...
/* The form/list code never needs to change when we upgrade storage */

(function () {
  const STORAGE_KEY = 'sov_transactions_v1';
  const MAX_NOTES_LENGTH = 200;

  // The 11 accounts (from your Day 7 sheet — single source of truth)
  const ACCOUNTS = [
  ...
  // 12 starter categories — will grow over time
  const CATEGORIES = [
  ...
  window.store = {
    ...
    addTransaction(input) {
      ...
      const tx = {
        id: genId(),
        type: input.type,                         // 'expense' | 'income' | 'transfer'
        amount: amount,
