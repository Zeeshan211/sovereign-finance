...
// v0.2.0  Finance Command Centre backend central action policy + lift criteria
//
// Contract:
// - Read-only only.
...
const VERSION = '0.2.0';
const ENFORCEMENT_VERSION = '0.2.0';
...
const runtimeStatus = getPath(coverage, 'runtime.status') || 'unknown';
const hasHardBlockers = hardBlockers.length > 0;
...
const runtimeNotRequired = runtimeStatus === 'unknown';
return [
checklist('transaction.save', 'Add Transaction save can lift only when write safety is proven.', [
...
item('write_safety_proven', 'Write safety dry-run/proof exists', writeSafetyPass, 'coverage.write_safety.status', 'Add a dry-run write-safety check before allowing transaction.save.'),
...
item('no_hard_blockers', 'No hard blockers exist', noHardBlockers, 'hard_blockers', 'Resolve all hard blockers shown in Command Centre.'),
item('write_safety_proven', 'Write safety dry-run/proof exists', writeSafetyPass, 'coverage.write_safety.status', 'Add a dry-run write-safety check before allowing bill.save.'),
...
item('no_hard_blockers', 'No hard blockers exist', noHardBlockers, 'hard_blockers', 'Resolve all hard blockers shown in Command Centre.'),
item('write_safety_proven', 'Write safety dry-run/proof exists', writeSafetyPass, 'coverage.write_safety.status', 'Add a dry-run write-safety check before allowing debt.save.'),
...
item('no_hard_blockers', 'No hard blockers exist', noHardBlockers, 'hard_blockers', 'Resolve all hard blockers shown in Command Centre.'),
item('write_safety_proven', 'Write safety dry-run/proof exists', writeSafetyPass, 'coverage.write_safety.status', 'Add a dry-run write-safety check before allowing reconciliation.declare.'),
item('reconciliation_source_readable', 'Reconciliation source remains readable', sourcePass('balances_api') || runtimeNotRequired, 'source_proofs.balances_api', 'Keep reconciliation source readable and do not hide truth rows.')
]),
checklist('salary.save', 'Salary save can lift only when salary baseline is proven and write safety exists.', [
item('no_hard_blockers', 'No hard blockers exist', noHardBlockers, 'hard_blockers', 'Resolve all hard blockers shown in Command Centre.'),
item('write_safety_proven', 'Write safety dry-run/proof exists', writeSafetyPass, 'coverage.write_safety.status', 'Add a dry-run write-safety check before allowing salary.save.'),
...
pages: {
status: pages.length ? 'registered' : 'unknown',
...
status: 'unknown',
score: 0,
note: 'No dry-run write safety exists yet. Mutating actions must stay blocked.'
},
runtime: {
status: 'unknown',
score: 0,
...
warnings.length === 0 &&
unknowns.length === 0 &&
Number(scores.write_safety || 0) > 0 &&
...
actions.push('Do not start backend mutating API enforcement until write safety and runtime checks exist.');
}
