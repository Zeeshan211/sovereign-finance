/* ─── Sovereign Finance · Reconciliation Catch-All API · v0.1.1 ───
 * Layer 5D Ship 1.
 *
 * Manual-verify reconciliation: operator declares "as of NOW, real bank balance
 * for account X is Y", system stores it + computes diff vs D1 computed balance.
 *
 * v0.1.1 fixes:
 *   - Existing account_id declarations UPDATE the existing row instead of failing
 *     D1 UNIQUE(reconciliation.account_id).
 *   - New account_id declarations INSERT normally.
 *   - Update path snapshots the previous reconciliation row before mutation.
 *   - Audit trail records create vs update clearly.
 *
 * Routes:
 *   GET    /api/reconciliation                        list all declarations + summary
 *   GET    /api/reconciliation/account/{account_id}   declaration history for one account
 *   POST   /api/reconciliation                        declare/update a balance
 *   POST   /api/reconciliation/{id}/note              append note
 */

import { json, audit, snapshot } from '../_lib.js';

/* ─── Helpers ─── */

function reconciliationId() {
  return 'recon_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
}

function toNumber(value, fallback = 0) {
  const n = Number(value);
  return Number.isFinite(n) ? n : fallback;
}

async function computeBalance(db, accountId) {
  const acc = await db
    .prepare(`SELECT opening_balance FROM accounts WHERE id = ?`)
    .bind(accountId)
    .first();

  if (!acc) return null;

  const r = await db
    .prepare(
      `SELECT
         COALESCE(SUM(CASE
           WHEN type = 'income' THEN amount
           WHEN type = 'transfer_in' THEN amount
           ELSE 0
         END), 0) AS credits,
         COALESCE(SUM(CASE
           WHEN type = 'expense' THEN amount
           WHEN type = 'transfer_out' THEN amount
           ELSE 0
         END), 0) AS debits
       FROM transactions
       WHERE account_id = ?
         AND (reversed_by IS NULL OR reversed_by = '')`
    )
    .bind(accountId)
    .first();

  const credits = toNumber(r?.credits);
  const debits = toNumber(r?.debits);

  return toNumber(acc.opening_balance) + credits - debits;
}

async function enrichDeclaration(db, row) {
  const currentBalance = await computeBalance(db, row.account_id);

  const acc = await db
    .prepare(`SELECT name, kind FROM accounts WHERE id = ?`)
    .bind(row.account_id)
    .first();

  const declared = toNumber(row.declared_balance);
  const storedDiff = toNumber(row.diff_amount);

  const liveDiff = currentBalance != null
    ? declared - currentBalance
    : null;

  return {
    ...row,
    account_name: acc?.name || row.account_id,
    account_kind: acc?.kind || null,
    current_d1_balance: currentBalance,
    diff_at_declaration: storedDiff,
    live_diff_vs_current_d1: liveDiff,
    is_clean: liveDiff != null && Math.abs(liveDiff) < 1,
  };
}

function rowSummary(row) {
  if (!row) return null;

  return {
    id: row.id,
    account_id: row.account_id,
    declared_balance: toNumber(row.declared_balance),
    declared_at: row.declared_at,
    declared_by: row.declared_by,
    notes: row.notes || null,
    diff_amount: toNumber(row.diff_amount),
  };
}

/* ─── Cloudflare Pages Function entry ─── */

export async function onRequest(context) {
  const { request, env, params } = context;
  const path = params.path;
  const segments = !path ? [] : (Array.isArray(path) ? path : [path]);
  const method = request.method;
  const db = env.DB;

  try {
    if (segments.length === 0) {
      if (method === 'GET') return await handleList(db);
      if (method === 'POST') return await handleCreateOrUpdate(db, request);
      return json({ ok: false, error: 'Method not allowed' }, 405);
    }

    if (segments.length === 2 && segments[0] === 'account') {
      if (method === 'GET') return await handleAccountHistory(db, segments[1]);
      return json({ ok: false, error: 'Method not allowed' }, 405);
    }

    if (segments.length === 2 && segments[1] === 'note') {
      if (method === 'POST') return await handleAddNote(db, segments[0], request);
      return json({ ok: false, error: 'Method not allowed' }, 405);
    }

    return json({
      ok: false,
      error: 'Not found. Available: GET /, POST /, GET /account/{id}, POST /{recon_id}/note',
    }, 404);
  } catch (e) {
    console.error('[reconciliation api]', e);
    return json({ ok: false, error: e.message || String(e) }, 500);
  }
}

/* ─── GET /api/reconciliation ─── */

async function handleList(db) {
  const rs = await db
    .prepare(`SELECT * FROM reconciliation ORDER BY declared_at DESC LIMIT 50`)
    .all();

  const rows = rs.results || [];
  const enriched = await Promise.all(rows.map(r => enrichDeclaration(db, r)));

  const latestByAccount = {};

  for (const e of enriched) {
    if (!latestByAccount[e.account_id]) {
      latestByAccount[e.account_id] = e;
    }
  }

  const accountsLatest = Object.values(latestByAccount);
  const cleanCount = accountsLatest.filter(a => a.is_clean).length;
  const driftedCount = accountsLatest.filter(a => !a.is_clean && a.live_diff_vs_current_d1 != null).length;

  return json({
    ok: true,
    version: 'v0.1.1',
    declarations: enriched,
    accounts_latest: accountsLatest,
    declarations_count: enriched.length,
    accounts_with_declarations: accountsLatest.length,
    clean_count: cleanCount,
    drifted_count: driftedCount,
  });
}

/* ─── GET /api/reconciliation/account/{account_id} ─── */

async function handleAccountHistory(db, accountId) {
  const acc = await db
    .prepare(`SELECT id, name FROM accounts WHERE id = ?`)
    .bind(accountId)
    .first();

  if (!acc) {
    return json({ ok: false, error: `Account ${accountId} not found` }, 404);
  }

  const rs = await db
    .prepare(
      `SELECT *
       FROM reconciliation
       WHERE account_id = ?
       ORDER BY declared_at DESC`
    )
    .bind(accountId)
    .all();

  const rows = rs.results || [];
  const enriched = await Promise.all(rows.map(r => enrichDeclaration(db, r)));
  const currentBalance = await computeBalance(db, accountId);

  return json({
    ok: true,
    version: 'v0.1.1',
    account_id: accountId,
    account_name: acc.name,
    current_d1_balance: currentBalance,
    declarations: enriched,
    declarations_count: enriched.length,
  });
}

/* ─── POST /api/reconciliation ─── */

async function handleCreateOrUpdate(db, request) {
  const body = await request.json().catch(() => ({}));

  const account_id = String(body.account_id || '').trim();
  const declared_balance = Number(body.declared_balance);
  const notes = String(body.notes || '').trim() || null;
  const declared_by = String(body.declared_by || 'operator').trim() || 'operator';

  if (!account_id) {
    return json({ ok: false, error: 'account_id is required' }, 400);
  }

  if (!Number.isFinite(declared_balance)) {
    return json({ ok: false, error: 'declared_balance is required and must be a number' }, 400);
  }

  const acc = await db
    .prepare(`SELECT id, name FROM accounts WHERE id = ?`)
    .bind(account_id)
    .first();

  if (!acc) {
    return json({ ok: false, error: `Account ${account_id} not found` }, 400);
  }

  const d1Balance = await computeBalance(db, account_id);
  const diff_amount = declared_balance - toNumber(d1Balance);
  const now = new Date().toISOString();

  const existing = await db
    .prepare(`SELECT * FROM reconciliation WHERE account_id = ?`)
    .bind(account_id)
    .first();

  if (existing) {
    return await updateExistingDeclaration(db, {
      existing,
      account_id,
      account_name: acc.name,
      declared_balance,
      declared_at: now,
      declared_by,
      notes,
      d1_balance_at_declaration: d1Balance,
      diff_amount,
    });
  }

  return await createNewDeclaration(db, {
    id: reconciliationId(),
    account_id,
    account_name: acc.name,
    declared_balance,
    declared_at: now,
    declared_by,
    notes,
    d1_balance_at_declaration: d1Balance,
    diff_amount,
  });
}

async function createNewDeclaration(db, data) {
  await db
    .prepare(
      `INSERT INTO reconciliation
        (id, account_id, declared_balance, declared_at, declared_by, notes, diff_amount)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
    .bind(
      data.id,
      data.account_id,
      data.declared_balance,
      data.declared_at,
      data.declared_by,
      data.notes,
      data.diff_amount
    )
    .run();

  await audit(db, {
    action: 'RECONCILIATION_DECLARE',
    entity_type: 'reconciliation',
    entity_id: data.id,
    details: {
      account_id: data.account_id,
      account_name: data.account_name,
      declared_balance: data.declared_balance,
      d1_balance_at_declaration: data.d1_balance_at_declaration,
      diff_amount: data.diff_amount,
      is_clean: Math.abs(data.diff_amount) < 1,
      notes: data.notes,
      mode: 'create',
      api_version: 'v0.1.1',
    },
    created_by: data.declared_by,
  });

  return json({
    ok: true,
    version: 'v0.1.1',
    id: data.id,
    account_id: data.account_id,
    declared_balance: data.declared_balance,
    d1_balance_at_declaration: data.d1_balance_at_declaration,
    diff_amount: data.diff_amount,
    is_clean: Math.abs(data.diff_amount) < 1,
    declared_at: data.declared_at,
    action: 'RECONCILIATION_DECLARE',
    mode: 'create',
  });
}

async function updateExistingDeclaration(db, data) {
  const snapId = await snapshot(db, {
    label: `recon_update_${data.existing.id}_${Date.now()}`,
    tables: ['reconciliation'],
    where: `id = '${data.existing.id}'`,
  });

  await db
    .prepare(
      `UPDATE reconciliation
       SET declared_balance = ?,
           declared_at = ?,
           declared_by = ?,
           notes = ?,
           diff_amount = ?
       WHERE account_id = ?`
    )
    .bind(
      data.declared_balance,
      data.declared_at,
      data.declared_by,
      data.notes,
      data.diff_amount,
      data.account_id
    )
    .run();

  await audit(db, {
    action: 'RECONCILIATION_DECLARE_UPDATE',
    entity_type: 'reconciliation',
    entity_id: data.existing.id,
    details: {
      account_id: data.account_id,
      account_name: data.account_name,
      previous: rowSummary(data.existing),
      declared_balance: data.declared_balance,
      d1_balance_at_declaration: data.d1_balance_at_declaration,
      diff_amount: data.diff_amount,
      is_clean: Math.abs(data.diff_amount) < 1,
      notes: data.notes,
      snapshot_id: snapId,
      mode: 'update_existing_account_id',
      api_version: 'v0.1.1',
    },
    created_by: data.declared_by,
  });

  return json({
    ok: true,
    version: 'v0.1.1',
    id: data.existing.id,
    account_id: data.account_id,
    declared_balance: data.declared_balance,
    d1_balance_at_declaration: data.d1_balance_at_declaration,
    diff_amount: data.diff_amount,
    is_clean: Math.abs(data.diff_amount) < 1,
    declared_at: data.declared_at,
    action: 'RECONCILIATION_DECLARE_UPDATE',
    mode: 'update_existing_account_id',
    snapshot_id: snapId,
  });
}

/* ─── POST /api/reconciliation/{id}/note ─── */

async function handleAddNote(db, id, request) {
  const body = await request.json().catch(() => ({}));
  const newNote = String(body.note || '').trim();

  if (!newNote) {
    return json({ ok: false, error: 'note is required' }, 400);
  }

  const existing = await db
    .prepare(`SELECT * FROM reconciliation WHERE id = ?`)
    .bind(id)
    .first();

  if (!existing) {
    return json({ ok: false, error: 'Reconciliation declaration not found' }, 404);
  }

  const stamp = new Date().toISOString().slice(0, 16).replace('T', ' ');
  const combined = existing.notes
    ? `${existing.notes}\n\n[${stamp}] ${newNote}`
    : `[${stamp}] ${newNote}`;

  const snapId = await snapshot(db, {
    label: `recon_note_${id}_${Date.now()}`,
    tables: ['reconciliation'],
    where: `id = '${id}'`,
  });

  await db
    .prepare(
      `UPDATE reconciliation
       SET notes = ?
       WHERE id = ?`
    )
    .bind(combined, id)
    .run();

  await audit(db, {
    action: 'RECONCILIATION_NOTE_APPEND',
    entity_type: 'reconciliation',
    entity_id: id,
    details: {
      previous_notes: existing.notes,
      appended: newNote,
      snapshot_id: snapId,
      api_version: 'v0.1.1',
    },
    created_by: 'web',
  });

  return json({
    ok: true,
    version: 'v0.1.1',
    id,
    action: 'RECONCILIATION_NOTE_APPEND',
    snapshot_id: snapId,
  });
}
