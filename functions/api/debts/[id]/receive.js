/* /api/debts/:id/receive — receive money owed to me */
/* Sovereign Finance v0.5.0-debts-money-truth-receive
 *
 * Only allowed for owed_to_me / owed / receivable.
 * Effect:
 * - creates income transaction into selected account
 * - increases paid_amount
 * - reduces remaining receivable
 */

const VERSION = "v0.5.0-debts-money-truth-receive";
  ...
  try {
    ...
    if (debt.direction !== "owed_to_me") {
      return json({
        ok: false,
        version: VERSION,
        error: "Use pay endpoint for i_owe debts.",
        direction: debt.direction,
        allowed_endpoint: `/api/debts/${debtId}/pay`
      }, 409);
    }
    ...
    const accountId = requireClean(body.account_id || body.destination_account_id, "account_id");
    const account = await resolveAccount(db, accountId);
    if (!account.ok) return json(account, account.status || 409);

    const date = normalizeDate(body.date || body.received_date || body.payment_date) || today();
    ...
    const txId = makeId("tx_debt_receive");
    const txNotes = [
    ...
    const proof = {
      ...
      account_effect: "selected_account_increases",
