...
export async function onRequestGet({ env, request }) {
  try {
    ...
    const debts = (result.results || []).map(normalizeDebtRow);
...
export async function onRequestPost({ env, request }) {
  try {
    ...
    if (action === "create" || !body.id) {
      const result = await createDebt(db, body);
      return json({ ok: true, version: VERSION, ...result });
    }

    if (action === "update") {
      const result = await updateDebt(db, body.id || body.debt_id, body);
      return json({ ok: true, version: VERSION, ...result });
    }

    if (action === "defer") {
      const result = await deferDebt(db, body.id || body.debt_id, body);
      return json({ ok: true, version: VERSION, ...result });
    }

    if (action === "payment" || action === "received") {
      const result = await recordDebtMovement(db, body.id || body.debt_id, body);
      return json({ ok: true, version: VERSION, ...result });
    }
...
async function createDebt(db, body) {
  ...
  const id = cleanText(body.id) || makeId("debt");
  const name = requireText(body.name, "name");
  const kind = normalizeKind(body.kind || body.direction || body.type);
  const original = requirePositiveNumber(body.original_amount ?? body.amount, "original_amount");
  const paid = numberOrZero(body.paid_amount);
  const dueDate = cleanText(body.next_due_date || body.due_date);
  const notes = cleanText(body.notes);
  const installment = optionalNumber(body.installment_amount);
  const frequency = cleanText(body.frequency) || "custom";
  ...
  const createLedgerEntry =
    body.create_ledger_entry === true ||
    body.ledger_mode === "with_ledger" ||
    Boolean(body.account_id && body.movement_date);
  ...
  await db.batch([debtInsert, transactionInsert]);
