...
export async function onRequestGet(context) {
  try {
    ...
    if (path[0] && !isReservedPath(path[0])) return getBillDetail(db, path[0], url);

    return getOverview(db, url);
  } catch (err) {
    return json(errorPayloadFromException('BILLS_GET_FAILED', err), 500);
  }
}
...
export async function onRequestPost(context) {
  try {
    ...
    return json({
      ...
      error: { code: 'UNSUPPORTED_ACTION', message: `Unsupported Bills action: ${action}` },
      supported_actions: ['create', 'pay', 'update', 'defer', 'repair']
    }, 400);
  } catch (err) {
    return json(errorPayloadFromException('BILLS_POST_FAILED', err), 500);
  }
}
...
export async function onRequestPut(context) {
  try {
    ...
    return json(errorPayloadFromException('BILLS_PUT_FAILED', err), 500);
  }
}
...
export async function onRequestDelete(context) {
  try {
    ...
    if (!billId) {
      return json({ ok: false, version: VERSION, error: { code: 'BILL_ID_REQUIRED', message: 'Bill id is required.' } }, 400);
    }
    const cols = await tableColumns(db, 'bills');
    const bill = await findBill(db, billId);
    if (!bill) {
      return json({ ok: false, version: VERSION, error: { code: 'BILL_NOT_FOUND', message: `Bill not found: ${billId}` } }, 404);
    }
    const updates = {};
    ...
    if (!Object.keys(updates).length) {
      return json({ ok: false, version: VERSION, error: { code: 'BILL_DELETE_UNSUPPORTED', message: 'Bills table has no supported soft-delete columns.' } }, 409);
    }
    await updateRow(db, 'bills', cols, updates, 'id = ?', [billId]);
    return json({ ok: true, version: VERSION, action: 'bill.delete', writes_performed: true, bill_id: billId });
  } catch (err) {
    return json(errorPayloadFromException('BILLS_DELETE_FAILED', err), 500);
  }
}
...
async function getOverview(db, url) {
  ...
  return json({
    ...
    contract: {
      ...
      pay_endpoint: '/api/bills/pay', update_endpoint: '/api/bills/update',
      defer_endpoint: '/api/bills/defer', repair_endpoint: '/api/bills/repair'
    }
  });
}
...
async function getHistory(db, url) {
  ...
  if (!billId) {
    return json({ ok: false, version: VERSION, error: { code: 'BILL_ID_REQUIRED', message: 'bill_id is required.' } }, 400);
  }
  const bill = await findBill(db, billId);
  if (!bill) {
    return json({ ok: false, version: VERSION, error: { code: 'BILL_NOT_FOUND', message: `Bill not found: ${billId}` } }, 404);
  ...
  const history = payments.map(payment => {
    ...
    return { ...payment, effective_paid: classified.effective_paid, ignore_reason: classified.ignore_reason, linked_transaction: tx || null };
  });
...
function buildCurrentCycle(input) {
  ...
  const paidAmount = round2(activePayments.reduce((sum, p) => sum + Number((p && p.amount) || 0), 0));
  const remainingAmount = round2(Math.max(0, amount - paidAmount));
  const status = remainingAmount <= 0 ? 'paid' : paidAmount > 0 ? 'partial' : 'unpaid';

  return {
    month, amount, paid_amount: paidAmount, remaining_amount: remainingAmount, status,
    payments: activePayments, ignored_payments: ignoredPayments,
    effective_payment_count: activePayments.length, ignored_payment_count: ignoredPayments.length,
    due_day: bill && bill.due_day, due_date: dueDateForMonth(month, bill && bill.due_day)
  };
}
...
function classifyPayment(payment, tx) {
  ...
  if (p.reversed_at || p.reversal_transaction_id) {
...
async function createBill(db, body, dryRun) {
  ...
  if (!name) return json({ ok: false, version: VERSION, error: { code: 'BILL_NAME_REQUIRED', message: 'Bill name is required.' } }, 400);
  if (amount == null || amount <= 0) return json({ ok: false, version: VERSION, error: { code: 'BILL_AMOUNT_REQUIRED', message: 'Bill amount must be greater than 0.' } }, 400);

  const row = {
    id, name, title: name, amount, expected_amount: amount,
    due_day: dueDay, frequency, category_id: categoryId,
    default_account_id: defaultAccountId || null, account_id: defaultAccountId || null,
    last_paid_date: null, last_paid_account_id: null, auto_post: 0,
    status: 'active', notes,
    created_at: nowSql(), updated_at: nowSql(), deleted_at: null
  };

  if (dryRun) {
    return json({ ok: true, version: VERSION, action: 'bill.create.dry_run', dry_run: true, writes_performed: false, bill: filterToColumns(row, cols) });
  }

  await insertRow(db, 'bills', cols, row);
  const bill = await findBill(db, id);
  return json({ ok: true, version: VERSION, action: 'bill.create', writes_performed: true, bill });
}
...
async function payBill(db, body, dryRun) {
  ...
  if (amount == null || amount <= 0) {
    return json({ ok: false, version: VERSION, error: { code: 'PAYMENT_AMOUNT_REQUIRED', message: 'Payment amount must be greater than 0.' } }, 400);
  }
  ...
  if (!accountId) {
    return json({
      ok: false, version: VERSION,
      error: { code: 'PAYMENT_ACCOUNT_REQUIRED', message: 'Payment account is required. Bills engine no longer silently defaults to Cash.' },
      bill_id: bill.id, bill_name: bill.name
    }, 400);
  }
  ...
  const transaction = {
    ...
    description: `Bill payment: ${bill.name}`, memo: `Bill payment: ${bill.name}`,
    fee_amount: 0, pra_amount: 0,
    created_at: createdAt, updated_at: createdAt,
    reversed_by: null, reversed_at: null, linked_txn_id: null, status: 'active'
  };

  const payment = {
    id: paymentId, bill_id: bill.id, bill_name_snapshot: bill.name,
    bill_month: month, month, cycle_month: month,
    amount, amount_paisa: toPaisa(amount), paid_amount: amount, paid_amount_paisa: toPaisa(amount),
    account_id: accountId, category_id: categoryId,
    paid_date: paidDate, payment_date: paidDate, date: paidDate,
    transaction_id: txId, txn_id: txId, ledger_transaction_id: txId,
    status: 'paid', notes,
    created_at: createdAt, updated_at: createdAt,
  ...
  if (dryRun) {
    return json({
      ok: true, version: VERSION, action: 'bill.pay.dry_run', dry_run: true, writes_performed: false,
      bill, transaction, payment,
  ...
  return json({
    ok: true, version: VERSION, action: 'bill.pay', writes_performed: true,
    bill_id: bill.id, payment_id: paymentId, transaction_id: txId,
    bill: overview ? (overview.bills || []).find(row => row.id === bill.id) || null : null,
...
async function updateBill(db, body, dryRun) {
  const billId = clean(body.bill_id || body.id);
  const bill = await findBill(db, billId);
  if (!bill) return json({ ok: false, version: VERSION, error: { code: 'BILL_NOT_FOUND', message: `Bill not found: ${billId}` } }, 404);
  ...
  if (body.amount !== undefined) {
    ...
    if (amount == null || amount <= 0) return json({ ok: false, version: VERSION, error: { code: 'INVALID_AMOUNT', message: 'Amount must be greater than 0.' } }, 400);
  ...
  if (!Object.keys(updates).length) return json({ ok: false, version: VERSION, error: { code: 'NO_SUPPORTED_FIELDS', message: 'No supported bill fields were supplied.' } }, 400);

  if (dryRun) return json({ ok: true, version: VERSION, action: 'bill.update.dry_run', dry_run: true, writes_performed: false, bill_id: bill.id, updates });

  await updateRow(db, 'bills', cols, updates, 'id = ?', [bill.id]);
  return json({ ok: true, version: VERSION, action: 'bill.update', writes_performed: true, bill: await findBill(db, bill.id) });
}
...
async function deferBill(db, body, dryRun) {
  ...
  if (!Object.keys(updates).length) return json({ ok: false, version: VERSION, error: { code: 'NO_SUPPORTED_DEFER_FIELDS', message: 'No supported defer fields were supplied.' } }, 400);

  if (dryRun) return json({ ok: true, version: VERSION, action: 'bill.defer.dry_run', dry_run: true, writes_performed: false, bill_id: bill.id, updates });

  await updateRow(db, 'bills', cols, updates, 'id = ?', [bill.id]);
  return json({ ok: true, version: VERSION, action: 'bill.defer', writes_performed: true, bill: await findBill(db, bill.id) });
}
...
async function repairReversedPayments(db, body, dryRun) {
  ...
  if (!bad.length) {
    return json({
      ok: true, version: VERSION, action: 'bill.repair_reversed_payments',
      dry_run: Boolean(dryRun), writes_performed: false, bad_payments_found: 0,
  ...
  if (dryRun) {
    return json({
      ok: true, version: VERSION, action: 'bill.repair_reversed_payments.dry_run',
      dry_run: true, writes_performed: false, bad_payments_found: bad.length,
      bad_payments: bad.map(payment => {
        const tx = txnsById.get(payment.transaction_id);
        return {
          payment_id: payment.id, bill_id: payment.bill_id, bill_month: payment.bill_month,
          amount: payment.amount, account_id: payment.account_id, transaction_id: payment.transaction_id,
          transaction_reversed_by: tx ? tx.reversed_by : null, transaction_reversed_at: tx ? tx.reversed_at : null
        };
      })
    });
  }
  ...
  return json({
    ok: true, version: VERSION, action: 'bill.repair_reversed_payments', writes_performed: true,
    bad_payments_found: bad.length, bad_payments_repaired: bad.length,
...
async function loadBills(db) {
  ...
  const select = [
    ...
    col(cols, 'due_day'), col(cols, 'due_date'), col(cols, 'frequency'),
    col(cols, 'category_id'),
    firstExisting(cols, ['default_account_id', 'account_id'], 'default_account_id'),
    col(cols, 'last_paid_date'), col(cols, 'last_paid_account_id'),
    col(cols, 'status'), col(cols, 'deleted_at'), col(cols, 'notes'),
    col(cols, 'created_at'), col(cols, 'updated_at')
  ].filter(Boolean);
  const orderBy = cols.has('due_day') ? 'due_day IS NULL, due_day ASC, name ASC' : 'name ASC';
  const res = await db.prepare(`SELECT ${select.join(', ')} FROM bills ORDER BY ${orderBy}`).all();
...
async function findBill(db, billId) {
  ...
  const row = await db.prepare(`SELECT ${select.join(', ')} FROM bills WHERE id = ? LIMIT 1`).bind(billId).first();
...
async function loadBillPayments(db) {
  ...
  const select = [
    ...
    col(cols, 'account_id'), col(cols, 'category_id'),
    ...
    col(cols, 'status'), col(cols, 'reversed_at'),
    firstExisting(cols, ['reversal_transaction_id', 'reversed_by'], 'reversal_transaction_id'),
    col(cols, 'notes'), col(cols, 'created_at'), col(cols, 'updated_at')
  ].filter(Boolean);
  const res = await db.prepare(`SELECT ${select.join(', ')} FROM bill_payments`).all();
...
async function loadTransactionsById(db) {
  ...
  const select = [
    ...
    col(cols, 'amount'), col(cols, 'account_id'), col(cols, 'category_id'),
    firstExisting(cols, ['notes', 'description', 'memo'], 'notes'),
    col(cols, 'created_at'), col(cols, 'reversed_by'), col(cols, 'reversed_at'),
    firstExisting(cols, ['linked_txn_id', 'linked_transaction_id'], 'linked_txn_id')
  ].filter(Boolean);
  const res = await db.prepare(`SELECT ${select.join(', ')} FROM transactions`).all();
...
function normalizeBill(row) {
  return {
    ...
    due_day: normalizeDueDay(row.due_day), due_date: normalizeDate(row.due_date),
    ...
    deleted_at: clean(row.deleted_at), notes: clean(row.notes),
    created_at: clean(row.created_at), updated_at: clean(row.updated_at)
  };
}
...
function normalizePayment(row) {
  const amount = row.amount == null
    ...
    : money(row.amount);
  return {
    id: clean(row.id), bill_id: clean(row.bill_id), bill_month: clean(row.bill_month),
    amount: amount == null ? 0 : round2(amount),
    amount_paisa: row.amount_paisa == null ? null : Number(row.amount_paisa),
    account_id: clean(row.account_id), category_id: clean(row.category_id),
    paid_date: normalizeDate(row.paid_date), transaction_id: clean(row.transaction_id),
...
function normalizeTxn(row) {
  return {
    id: clean(row.id), type: clean(row.type).toLowerCase(),
    amount: money(row.amount) || 0,
    account_id: clean(row.account_id), category_id: clean(row.category_id),
    notes: clean(row.notes), created_at: clean(row.created_at),
    reversed_by: clean(row.reversed_by), reversed_at: clean(row.reversed_at),
...
function buildEmbeddedHealth(input) {
  ...
  for (const payment of payments || []) {
    ...
    if (!payment.transaction_id || !tx) {
      orphans.push({
        payment_id: payment.id, bill_id: payment.bill_id,
  ...
  return {
    ...
    payment_rows: (payments || []).length, orphans, active_payment_reversed_txn_mismatch,
    missing_reversal_txn: [], duplicate_bill_month_amount: [], amount_mismatches: []
  };
}
...
function isActiveBill(bill) {
  const status = clean((bill && bill.status) || 'active').toLowerCase();
  return status !== 'deleted' && status !== 'archived' && status !== 'inactive' && !(bill && bill.deleted_at);
}
...
function isDryRun(url, body) {
  return url.searchParams.get('dry_run') === '1' || url.searchParams.get('dry_run') === 'true' ||
    body.dry_run === true || body.dry_run === '1' || body.dry_run === 'true';
}
...
function col(cols, name) { return cols.has(name) ? name : null; }
...
function filterToColumns(row, cols) {
  const out = {};
  for (const [key, value] of Object.entries(row)) if (cols.has(key)) out[key] = value;
  return out;
}
function prepareInsert(db, table, cols, row) {
  ...
  return db.prepare(`INSERT INTO ${table} (${keys.join(', ')}) VALUES (${keys.map(() => '?').join(', ')})`).bind(...keys.map(key => filtered[key]));
}
async function insertRow(db, table, cols, row) { await prepareInsert(db, table, cols, row).run(); }
function prepareUpdate(db, table, cols, updates, whereSql, whereValues) {
  const filtered = filterToColumns(updates, cols);
  const keys = Object.keys(filtered);
  if (!keys.length) return null;
  return db.prepare(`UPDATE ${table} SET ${keys.map(key => `${key} = ?`).join(', ')} WHERE ${whereSql}`).bind(...keys.map(key => filtered[key]), ...(whereValues || []));
}
...
function monthFromDate(value) { const date = normalizeDate(value); return date ? date.slice(0, 7) : currentMonth(); }
...
function appendNote(existing, addition) {
  const base = clean(existing); const next = clean(addition);
