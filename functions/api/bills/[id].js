...
export async function onRequestPut({ env, params, request }) {
  return handleWrite(env, params.id, request);
}
...
async function handleWrite(env, id, request) {
  try {
    const db = requireDb(env);
    await ensureBillPayments(db);
    ...
    if (action === "defer") {
      const result = await deferBill(db, id, body);
      return json({ ok: true, version: VERSION, ...result });
    }

    if (action === "archive") {
      const result = await setBillStatus(db, id, "archived", body.notes);
      return json({ ok: true, version: VERSION, ...result });
    }

    if (action === "reactivate") {
      const result = await setBillStatus(db, id, "active", body.notes);
      return json({ ok: true, version: VERSION, ...result });
    }

    const result = await updateBill(db, id, body);
    return json({ ok: true, version: VERSION, ...result });
  } catch (err) {
    return json({ ok: false, version: VERSION, error: err.message }, 500);
  }
