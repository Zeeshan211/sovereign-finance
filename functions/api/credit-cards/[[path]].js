...
async function computeCardOutstanding(db, accountId) {
  const r = await db.prepare(
    `SELECT account_id, type, amount, amount_paisa, transfer_to_account_id, reversed_by, reversed_at, notes
     FROM transactions WHERE (account_id = ? OR transfer_to_account_id = ?) ORDER BY date ASC`
  ).bind(accountId, accountId).all();

  const rows = (r.results || []).filter(t => !isReversalRow(t));
  let balance = 0;

  for (const txn of rows) {
    const amt    = txn.amount_paisa != null ? txn.amount_paisa : Math.round((txn.amount || 0) * 100);
    const type   = (txn.type || '').toLowerCase();
    const isSrc  = txn.account_id === accountId;
    const isTgt  = txn.transfer_to_account_id === accountId;

    if ((type === 'transfer' || type === 'cc_payment') && txn.transfer_to_account_id) {
      if (isSrc) balance -= amt;
      if (isTgt) balance += amt;
      continue;
    }
    if (!isSrc) continue;
    if (['income', 'salary', 'borrow', 'opening'].includes(type)) { balance += amt; continue; }
    if (['expense', 'cc_spend', 'atm', 'repay'].includes(type))   { balance -= amt; }
  }

  const outstanding = Math.max(0, -balance);
  return { balance, paisa: outstanding, pkr: outstanding / 100 };
}
